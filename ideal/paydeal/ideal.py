...
SECURE_PATH = settings.IDEAL_SECURE_PATH
"""
...
#       return htmlspecialchars(strtr($text, $trans), ENT_QUOTES)
    ...
    def loadConfig( self ):
        ...
        RELEVANT_SETTINGS = [
                ...
                'IDEAL_CERTIFICATE0',
                'IDEAL_ACQUIRERURL',
                'IDEAL_ACQUIRERTIMEOUT',
                'IDEAL_MERCHANTID',
                'IDEAL_SUBID',
                'IDEAL_MERCHANTRETURNURL',
                'IDEAL_EXPIRATIONPERIOD',
                'IDEAL_LOGFILE',
                'IDEAL_TRACELEVEL'
                ]

        for setting in RELEVANT_SETTINGS:
            dConfData[setting[6:].strip().upper()] = getattr(settings, setting)
